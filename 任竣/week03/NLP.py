...
class TorchModel(nn.Module):
    def __init__(self, vector_dim, sentence_length, vocab):
        ...
        self.rnn = nn.RNN(vector_dim, vector_dim, batch_first=True)  #池化层
...
def evaluate(model, vocab, sample_length):
    ...
    with torch.no_grad():
        y_pred = model(x)  #模型预测
        predict_class = torch.argmax(y_pred, dim=1)  # 每行最大值的索引 0~6

        for pred, true in zip(predict_class, y):
...
def predict(model_path, vocab_path, input_strings):
    ...
    for i, input_string in enumerate(input_strings):
        probs = result[i]  # 7维概率向量
        class_idx = torch.argmax(probs).item()  # 取最大概率的索引

        # ✅ 格式化输出
        if class_idx < sentence_length:
            pos_str = f"位置{class_idx}"
        else:
            pos_str = "不存在"

        print("输入：%s, 预测：%s, 各类概率：%s" % (
            input_string,
            pos_str,
...
if __name__ == "__main__":
    main()
    test_strings = ["我你个他大哥", "你哥打他了我", "我是你哥大的", "我的困你他个"]
