...
class MultiClassficationModel(nn.Module):
    ...
    def forward(self, x, y=None):
        ...
        y_pred = self.linear(x)  # (batch_size, 1) -> (batch_size, 1)
        if y is not None:
...
def evaluate(model):
    ...
    with torch.no_grad():
        ...
        # for y_t , y_p in zip(y_pred, y):
        #     if torch.argmax(y_t) == int(y_p):
        #         correct += 1
        #     else:
        #         wrong += 1
        a_class = torch.argmax(y_pred, 1)    # 预测类别
        for y_t, y_p in zip(a_class, y):
            if int(y_t) == int(y_p):
                correct += 1
            else:
                wrong += 1
...
if __name__ == "__main__":
    main()
    test_vec = [[0.77889086,0.59869675,0.45893459,0.03504317,0.99920843],
                [0.45963533,0.8824256,0.65758807,0.24560434,0.5490681],
